...
AADynamicPawnController::AADynamicPawnController() : index{0} {
	...
	DataTable = LoadObject<UDataTable>(nullptr, TEXT("/Script/Engine.DataTable'/Game/BluePrints/PawnClassSelector.PawnClassSelector'"));
		...
		if (SpawnedCharacter) {
			...
			if (PawnDataTableRow->Charachter == EPawn::FPP) {
				if (Cast<AAFirstPersonPawn>(SpawnedCharacter)) {
					GEngine->AddOnScreenDebugMessage(-1, 2, FColor::Red, TEXT("Hiiiiiii"));
